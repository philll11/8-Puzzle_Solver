...
string breadthFirstSearch_with_VisitedList(string const initialState, string const goalState, int &numOfStateExpansions, int &maxQLength, float &actualRunningTime)
{
	...
	while (!Q.empty())
	{
		if (((float)(clock() - startTime) / CLOCKS_PER_SEC) > 3)
		{
			return "TIME_OUT";
		}
		...
		if (toExpand->canMoveUp() && toExpand->getPath().back() != 'D')
		{
			...
			{
				if (Visited_List.find(descendant->getHashedState()) == Visited_List.end())
				{ // Checks whether descendant is not in Visited List
					Q.push(descendant);
...
string progressiveDeepeningSearch_No_VisitedList(string const initialState, string const goalState, int &numOfStateExpansions, int &maxQLength, float &actualRunningTime, int ultimateMaxDepth)
{
	...
	{
		if (((float)(clock() - startTime) / CLOCKS_PER_SEC) > 100)
		{
			return "TIME_OUT";
		}
		...
		while (!stackQueuedStates.empty())
		{
			...
			//In put on the stack in reverse order so they can be retreived in the correct order
			...
			if (maxQLength < stackQueuedStates.size())
				maxQLength = stackQueuedStates.size();
...
string uniformCost_ExpandedList(string const initialState, string const goalState, int &numOfStateExpansions, int &maxQLength, float &actualRunningTime, int &numOfDeletionsFromMiddleOfHeap, int &numOfLocalLoopsAvoided, int &numOfAttemptedNodeReExpansions)
{
	...
	while (!Q.empty())
	{
		if (((float)(clock() - startTime) / CLOCKS_PER_SEC) > 45)
		{
			return "TIME_OUT";
		}
		...
		if (Expanded_List.find(toExpand->getHashedState()) == Expanded_List.end())
		{
			...
			if (toExpand->canMoveUp() && toExpand->getPath().back() != 'D')
			{
				...
				if (toExpand->checkSearchNode(descendant->getHashedState()))
				{
					if (Expanded_List.find(descendant->getHashedState()) == Expanded_List.end())
					{ // Checks whether descendant is not in Expanded List
						bool descendInQ = false;
						for (unsigned i = 0; i < Q.size(); ++i)
						{ // Checks whether descendant is in Q
							if (descendant->getHashedState() == Q[i]->getHashedState())
							{ // If descendant not in Q, descendInQ will never be true
								descendInQ = true;
								if (descendant->getPathLength() < Q[i]->getPathLength())
								{ // Compares path lengths and keeps shorter path
									Q.erase(Q.begin() + i);
...
string aStar_ExpandedList(string const initialState, string const goalState, int &numOfStateExpansions, int &maxQLength, float &actualRunningTime, int &numOfDeletionsFromMiddleOfHeap, int &numOfLocalLoopsAvoided, int &numOfAttemptedNodeReExpansions, heuristicFunction heuristic)
{
	...
	while (!Q.empty())
	{
		if (((float)(clock() - startTime) / CLOCKS_PER_SEC) > 40)
		{
			return "TIME_OUT";
		}
		...
		if (Expanded_List.find(toExpand->getHashedState()) == Expanded_List.end())
		{
			...
			if (toExpand->canMoveUp() && toExpand->getPath().back() != 'D')
			{
				...
				if (toExpand->checkSearchNode(descendant->getHashedState()))
				{
					...
					if (it != Expanded_List.end())
					{ // If descendant in EL, compare F costs
						descendInEL = true;
						if (descendant->getFCost() < it->second)
						{							 // Checks whether descendant F cost lower than state in EL
					...
					if (!descendInEL)
					{ // If descendant not found in EL, then we want to add it to Q if it is not in Q
						bool descendInQ = false;
						for (auto i = 0; i < Q.size(); ++i)
						{
							if (descendant->getHashedState() == Q[i]->getHashedState())
							{ // Checks whether descendant state is in Q
								if (descendant->getFCost() < Q[i]->getFCost())
								{							// Check whether descendant has lower F cost
